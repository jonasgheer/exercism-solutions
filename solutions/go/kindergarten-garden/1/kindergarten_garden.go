...
func NewGarden(diagram string, children []string) (*Garden, error) {
	...
	garden := Garden{plants: make(map[string][]string)}
	for _, line := range strings.Split(diagram, "\n") {
		...
		for i, child := range childrenCopy {
			...
			garden.plants[child] = append(garden.plants[child], first, second)
		}
	}
	return &garden, nil
}

var flowers = map[string]string{
	"G": "grass",
	"C": "clover",
	"R": "radishes",
	"V": "violets",
...
func (g *Garden) Plants(child string) ([]string, bool) {
	if plants, ok := g.plants[child]; ok {
		return plants, true
	}
	return nil, false
}
